...
Indicator.SPACE_COLOR = "rgb(186,186,186)";

Indicator.draw = function(timelog,taskHash)
{
    var fillSegments = [];
	var logedTasks = {};
	for (var i=0; i < timelog.logs.length; i++) {
		...
		var color = Indicator.SPACE_COLOR;
		if (task) {
			color = task.color;
	...
	fillSegments.push({color: Indicator.SPACE_COLOR, value: timelog.leftTime()});
	
	$('#legend').empty();
	for (var task_id in logedTasks){
		var task = logedTasks[task_id];
        $('#legend').append(Indicator._makeLegendElement(task.name, task.caculateHour(timelog)+"h", task.color, task.id));
	}
	var leftTimeInHour = Math.round(timelog.leftTime() /1000/60/60*10)/10;
	$('#legend').append(Indicator._makeLegendElement('left', leftTimeInHour+"h", Indicator.SPACE_COLOR));
    
    Indicator._drawSummaryGraph(fillSegments);
}


Indicator._drawSummaryGraph = function(segments)
{
    ...
    Indicator._fadeOutRect(ctx, x, y + h + 1, w, h, 0.5, 0.0);
}

Indicator._fadeOutRect = function(ctx, x, y, w, h, a1, a2)
{
    ctx.save();
...
Indicator._makeLegendElement = function(label, value, color,task_id)
{
    ...
    if (color) {
        ...
        Indicator._drawSwatch(swatch, color);
    }
...
Indicator._drawSwatch = function(canvas, color)
{
    var ctx = canvas.getContext("2d");
    ...
    Indicator._fadeOutRect(ctx, 0, 13, 13, 13, 0.5, 0.0);
}